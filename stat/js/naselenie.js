...
function loadNaselenie() {
     document.getElementById("maintxt").innerHTML = "Население"
    ...
    async function getNaselenie() {
        ...
        document.getElementById("t1").innerHTML = "Население КР";
   ...
   const reqDead ='https://cors-anywhere.herokuapp.com/http://stat.kg/ru/opendata/category/'+b+'/json'




    async function getDead() {
        const response = await fetch(reqDead);
        const data = await response.json();



        /*DEAD*/
        titleDead = data.title;

        deadnow = data.data[0].values[0].value;
        deadprev = data.data[0].values[1].value


        p2 = deadnow - deadprev;
        p2 = Math.round(p2 / deadprev * 100);



        document.querySelector('.card:nth-child(2) svg circle:nth-child(2)').style.strokeDashoffset = (440 - (440 * p2) / 100);

        document.getElementById("t2").innerHTML = titleDead;
        document.getElementById("n2").innerHTML = deadnow;
    ...
    async function getLife() {
        ...
        document.getElementById("t4").innerHTML = "Прод.жизни";
